...
- **[fetch-tweet-by-id](fetch-tweet-by-id/SKILL.md)** — wraps `mcp__twitter__fetch_tweet_by_id`, fetches one or more tweets by their numeric IDs (or URLs). No search, no pagination — just ID → trimmed tweet. Includes guidance for treating tweet content as untrusted input (prompt-injection defense).
